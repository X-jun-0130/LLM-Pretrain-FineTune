#!/usr/bin/env python3
"""
Qwen3.5 Layer Stacking Tool
直接复制模型中间层块实现参数量扩展（无需额外训练）

Qwen3.5 的层以 [linear_attn × 3, full_attn × 1] 为一个块（4层一组），
32层 = 8个块（编号0-7）。复制以完整块为单位。
...
用法:
    ...
    python layer_stacking.py \
        --src_dir /data1/Model-TH/Qwen3.5-9B \
        --dst_dir /data1/Model-TH/Qwen3.5-12B-stacked \
        --dup_groups 2,3,4,5

    # 3. 执行堆叠 → ~16B（32层→64层）
    python layer_stacking.py \
        --src_dir /data1/Model-TH/Qwen3.5-9B \
        --dst_dir /data1/Model-TH/Qwen3.5-16B-stacked \
        --dup_groups 0,1,2,3,4,5,6,7
"""
...
OUTPUT_PROJ_SUFFIXES = (
    ...
    '.mlp.down_proj.weight',          # MLP 的输出投影
)
...
COPY_FILES = [
    ...
    'LICENSE', 'README.md', 'configuration.json',
]


def parse_args():
    p = argparse.ArgumentParser(description='Qwen3.5 Layer Stacking Tool')
    p.add_argument('--src_dir', type=str, required=True,
                   help='原始模型目录路径')
    p.add_argument('--dst_dir', type=str, default=None,
                   help='输出模型目录路径（dry_run模式可省略）')
    p.add_argument('--dup_groups', type=str, default='2,3,4,5',
                   help='要复制的块编号，逗号分隔。8个块编号0-7，默认: 2,3,4,5')
...
def build_new_layer_types(old_layer_types: list[str], dup_groups: list[int]) -> list[str]:
    ...
    for block_idx in range(num_blocks):
        ...
        if block_idx in dup_groups:
            new_types.extend(block)  # 复制

    return new_types


def print_stacking_plan(old_num_layers, layer_map, new_num_layers, dup_groups,
                        old_layer_types, init_mode='identity', scale_factor=0.0):
    """打印可视化的堆叠计划。"""
    ...
    # 估算参数量变化（每层约 ~200M 参数）
    est_params_per_layer_m = 200
    est_old_total = 9.0
    est_new_total = est_old_total + (added_layers * est_params_per_layer_m / 1000)

    init_label = {'copy': '完全复制(⚠️会乱码)', 'identity': '直通/Identity(推荐)',
                  'scaled': f'缩放×{scale_factor}'}

    print("=" * 65)
    print("                  Qwen3.5 Layer Stacking Plan")
    print("=" * 65)
    ...
    print(f"  估算参数量:     ~{est_old_total:.1f}B → ~{est_new_total:.1f}B")
    print("-" * 65)
    ...
    for old_block in range(num_old_blocks):
        old_layers = f"L{old_block*4}-{old_block*4+3}"
        new_layers = f"L{new_block_idx*4}-{new_block_idx*4+3}"
        tag = ""
        print(f"    Block {old_block} ({old_layers})  →  NewBlock {new_block_idx} ({new_layers}) {tag}")
        new_block_idx += 1

        if old_block in dup_groups:
            new_layers = f"L{new_block_idx*4}-{new_block_idx*4+3}"
            print(f"    Block {old_block} ({old_layers})  →  NewBlock {new_block_idx} ({new_layers})  ← [COPY]")
            new_block_idx += 1
    ...
    for key, tensor in all_weights.items():
        ...
        if match:
            ...
            for new_idx in reverse_map[old_idx]:
                ...
                is_output_proj = suffix.endswith(OUTPUT_PROJ_SUFFIXES)
...
def save_sharded_safetensors(weights: OrderedDict, dst_dir: str, max_shard_bytes: int):
    ...
    for i, shard in enumerate(shards):
        if num_shards == 1:
            ...
            filename = f"model.safetensors-{i+1:05d}-of-{num_shards:05d}.safetensors"

        filepath = os.path.join(dst_dir, filename)
        print(f"    [{i+1}/{num_shards}] {filename} ({len(shard)} tensors, "
              f"{sum(v.numel() * v.element_size() for v in shard.values()) / 1024**3:.2f} GB)")
...
def main():
    ...
    old_num_layers = config['text_config']['num_hidden_layers']
    ...
    print_stacking_plan(old_num_layers, layer_map, new_num_layers, dup_groups,
                        old_layer_types, args.init_mode, args.scale_factor)
    ...
    if args.init_mode == 'identity':
        print()
        print("  💡 identity模式说明:")
        print("     复制的层初始为直通(不做任何变换)，模型行为等同原始模型。")
        print("     如需让新层学到医疗知识，后续可对新层做 CPT/SFT 微调。")
    print("=" * 65 + "\n")
